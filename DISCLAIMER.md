# ⚠️ Disclaimer / Haftungsausschluss

> 🚨 **Nutzung auf eigene Gefahr / Use at your own risk**

By using this software, you acknowledge, understand, and accept the following terms.
Mit der Nutzung dieser Software bestätigst du, dass du die folgenden Bedingungen verstehst und akzeptierst.

---

## 📜 MIT License Notice

This project is licensed under the **MIT License**.
The MIT License permits use, modification, distribution, and private/commercial use of this software, provided the original copyright and license notice are included.

⚠️ **Important:**
While the MIT License grants broad usage rights, it also explicitly provides the software **without warranty or liability**.

Eine Kopie der vollständigen MIT-Lizenz befindet sich in der Datei [`LICENSE`](./LICENSE).

---

## 🇩🇪 Deutsch

### ❗ Keine Gewährleistung

Diese Software wird **ohne jegliche Gewährleistung** bereitgestellt, weder ausdrücklich noch stillschweigend.
Dies umfasst unter anderem:

* Funktionalität
* Fehlerfreiheit
* Sicherheit
* Kompatibilität
* Systemstabilität
* Performance-Auswirkungen

---

### 🔐 Eigenverantwortung

Mit der Nutzung dieser Software erklärst du dich damit einverstanden, dass:

* alle Änderungen **auf eigenes Risiko** erfolgen
* insbesondere Eingriffe in:

  * Registry
  * Systemkonfiguration
  * Sicherheitsrichtlinien
  * Performance-Optimierungen
  * Netzwerkeinstellungen
    Risiken bergen können
* du eigenständig für:

  * Backups
  * Wiederherstellungspunkte
  * Systemschutzmaßnahmen
    verantwortlich bist

---

### 💥 Haftungsausschluss

Soweit gesetzlich zulässig, haftet der Autor **nicht für Schäden jeglicher Art**, insbesondere:

* Datenverlust
* Systemfehler oder Abstürze
* Hardware-Schäden
* Sicherheitsprobleme
* Instabilität
* Inkompatibilitäten
* Fehlkonfigurationen
* indirekte oder Folgeschäden

---

### ⚖️ Rechtlicher Hinweis

Dieser Haftungsausschluss gilt im Rahmen der jeweils anwendbaren gesetzlichen Bestimmungen.
Zwingende gesetzliche Haftungsregelungen bleiben unberührt.

---

### 🧪 Empfehlung

Vor risikobehafteten Änderungen wird empfohlen:

* Nutzung in einer VM/Testumgebung
* Erstellung eines Systemabbilds
* Registry-Backup
* Erstellung eines Wiederherstellungspunkts

---

## 🇬🇧 English

### ❗ No Warranty

This software is provided **"as is"**, without warranties of any kind, express or implied, including but not limited to:

* Functionality
* Reliability
* Security
* Compatibility
* System stability
* Performance impact

---

### 🔐 User Responsibility

By using this software, you agree that:

* all actions are performed **at your own risk**
* system-level modifications involving:

  * Registry
  * Security settings
  * Performance tuning
  * Network configuration
    may introduce risks
* you are solely responsible for:

  * Backups
  * Restore points
  * System recovery measures

---

### 💥 Limitation of Liability

To the fullest extent permitted by law, the author shall **not be liable for any damages**, including but not limited to:

* Data loss
* System instability or crashes
* Hardware damage
* Security vulnerabilities
* Misconfiguration
* Performance degradation
* Indirect or consequential damages

---

### ⚖️ Legal Notice

This limitation of liability applies within the scope of applicable law.
Mandatory statutory provisions remain unaffected.

---

### 🧪 Recommendation

Before applying advanced or high-risk changes, users are strongly encouraged to:

* Test in a virtual machine
* Create system backups
* Backup registry settings
* Generate restore points

---

## 🔗 Additional Notes

* This project includes modules that may alter Windows settings, registry values, security features, and performance configurations.
* Certain modules require administrative privileges.
* Users should carefully review all actions before execution.
* Security-sensitive or performance-altering features should be used only by informed users.

---

## 📌 Final Statement

> **You are fully responsible for how you use this software.**

> **Der Nutzer trägt die volle Verantwortung für sämtliche Auswirkungen der Nutzung.**

---

**PowerTools Suite**
Version 1.2 • ReAlNoMo
